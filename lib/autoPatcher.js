...
var WRITE_ALL = false; // if true, write all patches to the below file instead of incrementally applying new ones
var ALL_PATCH_FILE = 'allPatches.sql';

if (WRITE_ALL) {
    console.log('Will write patch sql file for all patches instead of applying incrimental updates');
}
...
function patch(config) {
    ...
    db = !WRITE_ALL ? new EzMySql(mysqlOptions) : null;
    ...
    async.auto({
        ...
        currentPatchLevel: ['ensureInfrastructure', function(next) {
            if (!WRITE_ALL) {
                ...
                next(null, {major: -1, minor: -1, str: '(none)'});
            }
        }],
        executePatches: ['currentPatchLevel', function(next, results) {
            ...
            var patches = [];
            var numApplied = 0;
            var writeStream = WRITE_ALL ? fs.createWriteStream(ALL_PATCH_FILE) : null;
            ...
            if (WRITE_ALL) {
                writeStream.write('use '+config.database+';'+os.EOL+os.EOL);
            }
            ...
            async.eachSeries(patches, function(patch, eachNext) {
                if (!WRITE_ALL) {
                    ...
                    console.log('Writing '+path.basename(patch.filePath));
                    fs.readFile(patch.filePath, function(err, contents) {
                        writeStream.write(contents+os.EOL+os.EOL);
                        eachNext(err);
                    });
                }
            }, function(err) {
        ...
        if (!WRITE_ALL) {
            ...
            console.log('Patches written to '+path.resolve(ALL_PATCH_FILE));
        }
    });
}
