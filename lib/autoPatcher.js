...
Autopatcher.prototype = {
    ...
    _ensureDbTrackingTableMatchesNumberOfPatchLevels: function(callback) {
        ...
        var sql = 'SELECT COUNT(*) as numPatchColumns \n'
                ...
                + '     AND column_name NOT IN ("file","date_applied") \n';
        ...
        this.db.queryOne(sql, function(err, resultRow){
            ...
            var trackingTableMatchesNumberOfPatchLevels = resultRow && resultRow.numPatchColumns
                                                      && resultRow.numPatchColumns === self.config.numberOfPatchLevels;
            if (trackingTableMatchesNumberOfPatchLevels) {
                return callback(null,null);
            }
            else {
                err = new Error(
                    'numberOfPatchLevels ('+self.config.numberOfPatchLevels+') does not match number of tracking columns in database_patches ('+resultRow.numPatchColumns+').\n'
                    + 'To use specified profile, please re-create your DB from scratch.'
                );
    ...
    _removePatchFilesAlreadyExecuted: function(allPatchFilesWithPatchLevelData, currentPatchLevelObject) {
        ...
        allPatchFilesWithPatchLevelData.forEach(function(patchFile) {
            var patchComparisonResult = self._comparePatchLevels(patchFile, currentPatchLevelObject)
            var patchFileShouldBeExecuted = patchComparisonResult > 0;
    ...
    _sortPatchFilesToExecute: function(patchFilesToExecute) {
        var self = this;
        patchFilesToExecute.sort(function(patchFileA, patchFileB){
            return self._comparePatchLevels(patchFileA, patchFileB);
        });
    ...
    /**
     * Returns positive number if patchA > patchB, negative number if patchB < patchA, and 0 if patch levels are equal
     *
     * @param patchLevelA - object with trackingTablePatchLevelColumns defined e.g., {major: $major, minor: $minor, level3: $level3}
     * @param patchLevelB - object with trackingTablePatchLevelColumns defined e.g., {major: $major, minor: $minor, level3: $level3}
     * @private
     */
    _comparePatchLevels: function(patchLevelA, patchLevelB) {
        for(var i=0; i<this.trackingTablePatchLevelColumns.length; i++) {
            var column = this.trackingTablePatchLevelColumns[i];
            var delta = patchLevelA[column] - patchLevelB[column];
            if (delta !== 0) {
                return delta;
            }
        }
