 ...
 * @param attempt integer presenting the attempt number for executing the query (only retries if SQL connection dies)
 */
EzMySql.prototype.query = function(sql, callback, attempt) {
    ...
    if(attempt < 1000) {
        ...
        async.auto({
            ...
            if (err) {
                if(err.code === 'PROTOCOL_CONNECTION_LOST' || err.code === 'ECONNREFUSED') {
                    return self.query(sql, callback, attempt+1);
                }
        ...
        callback(new Error("MySQL connection error: PROTOCOL_CONNECTION_LOST"), false);
    }
};
