...
EzMySql.prototype.query = function(sql, callback) {
    ...
    async.auto({
        ...
        query: ['getConnection', function(next, results) {
            ...
            // Ensure that PROTOCOL_CONNECTION_LOST errors do not break autopatcher; retry connection
            self.replaceConnectionOnDisconnect(results.getConnection);
        ...
        if (connection) {
            // Remove the error listeners on successful query so that we don't end up with a memory leak / going over listener limit
 ...
 *
 * Code found here: https://www.exratione.com/2013/01/nodejs-connections-will-end-close-and-otherwise-blow-up/
 */
EzMySql.prototype.replaceConnectionOnDisconnect = function (connection) {
    ...
    connection.on('error', function (err) {
        ...
        self.replaceClientOnDisconnect(connection);
